^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Changelog for package diff_drive_control
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

0.6.10 (2024-03-18)
-------------------
* Mod: Set 'publish_cmd' param to true in diff_drive_control/config
  - With this change the diff drive controller will output the final cmd_vel to /diff_drive_velocity_controller/cmd_vel_out after any filters are applied (e.g., speed/acceleration limits).
...
0.6.3 (2022-05-16)
------------------
* Enable subst_value when loading config_extras. (`#226 <https://github.com/diff_drive/diff_drive/issues/226>`_)
* Remove whitespace
...
* Check launch file only if testing
  When building diff_drive_control, diff_drive_description, diff_drive_navigation or
  diff_drive_viz without tests, CMake fails as it does not find
...
* Made multimaster not come up by default in diff_drive_control
* [diff_drive_control] Fixed typo.
* Updated the rolling window size for more responsive control
* Fixed typo in URLs.
* Added dependency on diff_drive_description to diff_drive_control/package.xml
...
0.0.1 (2015-01-12)
------------------
* Initial development of diff_drive_control for Husky indigo release
* Contributors: Paul Bovbel
